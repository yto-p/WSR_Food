package com.app.resaless.fragment;

import android.os.Bundle;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.app.resaless.R;
import com.app.resaless.activity.MainActivity;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.fragment.app.Fragment;

public class SauceFragment extends Fragment {

    private View view;

    public static SauceFragment newInstance(MainActivity mainActivity, ViewGroup container){
        SauceFragment sauceFragment = new SauceFragment();
        sauceFragment.view = mainActivity.getLayoutInflater().inflate(R.layout.fragment_sauce, container, false);
        return sauceFragment;
    }

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return view;
    }
}
